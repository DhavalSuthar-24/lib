...
import { filter, isArray, isEmpty, isNil, isNilOrEmpty, isUndefined, omit, reduce, toPairs, validateUuid } from "@bit-core-api/shared-utils-lib";
  ...
  implements IReadOnlyRepo<T, TKey, TPageableFilter, TFilter>
{
  ...
  public get specialFilterFields(): (keyof TPageableFilter)[] {
    return ["$ids", "$order", "$orderBy", "$page", "$perPage", "$resolveImages"];
  }
